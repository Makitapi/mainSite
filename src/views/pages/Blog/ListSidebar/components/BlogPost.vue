<template>
  <b-col lg="9" class="pe-lg-4 pe-xl-5">
    <h1 class="pb-3 pb-lg-4">Blog list with sidebar</h1>
    <article class="row g-0 border-0 mb-4" v-for="(post, idx) in blogPostList" :key="idx" :class="idx && 'pt-3 pt-sm-0'">
      <a class="col-sm-5 bg-repeat-0 bg-size-cover bg-position-center rounded-5" :to="{ name: post.link.name }" style="min-height: 14rem" :style="{ backgroundImage: `url('${post.image}')` }" aria-label="Post image"></a>
      <b-col sm="7">
        <div class="pt-4 pb-sm-4 ps-sm-4 pe-lg-4">
          <h3>
            <router-link :to="{ name: post.link.name }">{{ post.title }}</router-link>
          </h3>
          <p class="d-sm-none d-md-block">{{ post.expert }}</p>
          <div class="d-flex flex-wrap align-items-center mt-n2">
            ...
            <span class="fs-sm text-body-secondary mt-2">{{ post.date }}</span>
            <span class="fs-xs opacity-20 mt-2 mx-3">|</span>
            <a class="badge text-nav fs-xs border mt-2" href="#">{{ post.category }}</a>
          </div>
        </div>
      </b-col>
    </article>
    <!-- Pagination -->
    <b-row class="gy-3 align-items-center mt-lg-5 pt-2 pt-md-4 pt-lg-0">
...
import { ref } from 'vue'
import { blogPostList } from '@/views/pages/Blog/ListSidebar/data'
