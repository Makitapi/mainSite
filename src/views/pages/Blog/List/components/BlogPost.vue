<template>
  <article class="row g-0 border-0 mb-4" v-for="(post, idx) in blogPostList" :key="idx" :class="post.shares == 6 ? '' : 'pt-3 pt-sm-0'">
    <a class="col-sm-5 col-lg-4 bg-repeat-0 bg-size-cover bg-position-center rounded-5" :to="{ name: 'blog.post-1' }" style="min-height: 16rem" :style="{ backgroundImage: `url('${post.image}')` }" aria-label="Post image"></a>
    <b-col sm="7" lg="8">
  ...
  <!-- Pagination -->
  <b-row class="gy-3 align-items-center mt-lg-5 pt-2 pt-md-3 pt-lg-0 mb-md-2 mb-xl-4">
...
import { blogPostList } from '@/views/pages/Blog/List/data'
