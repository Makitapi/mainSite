<template>
  ...
  <div class="mb-lg-5 mb-4">
    <article class="position-relative d-flex align-items-center mb-4" v-for="(item, idx) in trendingPost" :key="idx">
      <img class="rounded" :src="item.image" width="92" alt="Post image" />
      <div class="ps-3">
        <h4 class="h6 mb-2">
          <router-link class="stretched-link" :to="{ name: item.link.name }">{{ item.title }}</router-link>
        </h4>
  ...
  <div class="position-relative mb-3">
    ...
    <img class="rounded-5" :src="banner" alt="Banner" />
  </div>
</template>

<script setup lang="ts">
import { trendingPost } from '@/views/pages/Blog/GridSidebar/data'
