<template>
  <b-container class="py-5 mb-lg-3 mb-xl-4 mt-xl-3 my-xxl-5">
    <h2 class="h1 text-center pt-2 pt-sm-3 pt-md-4 pt-lg-5 mt-xl-2 mb-5">Meet our speakers</h2>
    <Simplebar class="mb-4 mb-lg-5 pb-4 px-3">
      <b-row class="row-cols-lg-4 flex-nowrap flex-lg-wrap gy-lg-5 mx-n3">
        <b-col v-for="(item, index) in speakers" :key="index">
          <a class="d-block text-center text-decoration-none" href="#" style="min-width: 210px">
            <img class="rounded-circle" :src="item.image" width="120" alt="Avatar" />
            <h3 class="h5 pt-4 mb-1">{{ item.name }}</h3>
            <p class="text-body-secondary mb-0">{{ item.designation }}</p>
          </a>
        </b-col>
      </b-row>
    </Simplebar>

    <b-card no-body class="bg-primary overflow-hidden py-2 py-sm-3 py-md-4 mb-sm-2 mb-md-4">
      <div class="bg-dark position-absolute top-0 start-0 w-100 h-100 opacity-5"></div>
      <div class="position-absolute top-0 start-0 w-100 h-100 opacity-15" :style="{ backgroundImage: `url(${noise})`, mixBlendMode: 'soft-light' }"></div>
      <b-card-body class="position-relative d-sm-flex align-items-center justify-content-between z-2 text-center">
...
import { speakers } from '@/views/landings/ConferenceEvent/data'
