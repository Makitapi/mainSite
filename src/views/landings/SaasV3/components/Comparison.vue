<template>
  <section class="d-flex w-100 position-relative overflow-hidden">
    <div class="position-relative flex-xl-shrink-0 z-5 start-50 translate-middle-x my-n1" style="max-width: 1920px">
      <div class="mx-md-n5 mx-xl-0">
        <div class="mx-n4 mx-sm-n5 mx-xl-0">
          <div class="mx-n5 mx-xl-0">
            <ImgComparisonSlider class="mx-n5 mx-xl-0">
              <!-- eslint-disable vue/no-deprecated-slot-attribute -->
              <img :src="lightMode" slot="first" alt="Light Mode" />

              <img class="d-dark-mode-none" slot="second" :src="darkModeLight" alt="Dark Mode" />
              <img class="d-none d-dark-mode-block" slot="second" :src="darkModeDark" alt="Dark Mode" />

              <div style="width: 42px" slot="handle">
                <svg class="text-primary rounded-circle" width="42" height="42" viewBox="0 0 42 42" xmlns="http://www.w3.org/2000/svg">
                  <g>
                    <circle fill="currentColor" cx="21" cy="21" r="21"></circle>
                  </g>
                  <path fill="white" d="M25.5019 19.7494H15.9147V15.9146L11.1211 20.7081L15.9147 25.5017V21.6669H25.5019V25.5017L30.2955 20.7081L25.5019 15.9146V19.7494Z"></path>
                </svg>
              </div>
              <!-- eslint-enable -->
            </ImgComparisonSlider>
          </div>
        </div>
      </div>
    </div>
    <div class="position-absolute top-0 start-0 w-50 h-100 bg-primary"></div>
    <div class="position-absolute top-0 end-0 w-50 h-100 d-dark-mode-none" style="background-color: #f6f9fc"></div>
    <div class="position-absolute top-0 end-0 w-50 h-100 d-none d-dark-mode-block" style="background-color: #171a1e"></div>
  </section>
</template>

<script setup lang="ts">
import { ImgComparisonSlider } from '@img-comparison-slider/vue'
import lightMode from '@/assets/img/landing/saas-3/comparison/light-mode.jpg'
import darkModeLight from '@/assets/img/landing/saas-3/comparison/dark-mode-light.jpg'
import darkModeDark from '@/assets/img/landing/saas-3/comparison/dark-mode-dark.jpg'
</script>
