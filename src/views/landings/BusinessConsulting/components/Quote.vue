<template>
  <section class="overflow-hidden">
    <b-container class="pb-5 pt-3 pt-md-4 pt-lg-4 my-xl-3 my-xxl-5">
      <b-row class="pb-md-3 pb-lg-4 mb-2 mb-lg-3">
        <b-col md="5" class="d-flex flex-column align-items-center mb-4 mb-md-0">
          <div class="position-relative" style="max-width: 416px">
            <img class="d-block position-relative z-2" :src="ceoImage" alt="Nicolas Black" />
            <div class="bg-primary position-absolute start-0 bottom-0 w-100" style="height: 85.4%; border-radius: 2.25rem 8rem 2.25rem 2.25rem"></div>
          </div>
          <div class="text-center text-md-start pt-3 mt-3">
            ...
            <div class="position-relative pt-4" style="max-width: 500px">
              ...
              <p class="fs-xl position-relative z-2 mb-0">The main task of our organization is <strong>to assess the state of your enterprise now and how you can achieve your goals.</strong> Self-assessment of current activities and review of processes formulate decisions to achieve the desired outcome.</p>
            </div>
          </div>
        </b-col>

        <b-col md="7" lg="6" xxl="5" class="offset-lg-1 offset-xxl-2">
          <div class="ps-md-4 ps-lg-0">
            <div class="position-relative d-flex justify-content-end my-3 py-1" data-aos="fade-left" data-aos-easing="ease-out-back" v-for="(step, idx) in steps" :key="idx" :data-aos-delay="step.delay">
              <div class="btn btn-lg btn-icon btn-primary fs-xl fw-bold rounded-circle position-absolute top-50 start-0 translate-middle-y z-2 pe-none">
                {{ idx + 1 }}
              </div>
              <b-card no-body class="border-0 ps-3" style="width: calc(100% - 1.75rem)">
                <b-card-body class="ps-4">
                  <h3 class="h5 card-title pb-2 mb-1">{{ step.title }}</h3>
...
import { steps } from '@/views/landings/BusinessConsulting/data'
import ceoImage from '@/assets/img/landing/business-consulting/ceo.png'
</script>
