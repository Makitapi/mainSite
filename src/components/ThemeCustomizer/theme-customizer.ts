import { colorKeys, type ColorType, type CustomizerType } from '@/components/ThemeCustomizer/data'
import { pascalCaseToKebabCase } from '@/helpers/change-casing'
...
export const getConfigFromStorage = () => localStorage.getItem(customizerConfigKey)

export const setConfigToStorage = (value: string) => localStorage.setItem(customizerConfigKey, value)

export const removeConfigFromStorage = () => localStorage.removeItem(customizerConfigKey)
...
export const changeFontLink = (link: string) => {
  ...
  if (headLinkElement && headLinkElement instanceof HTMLLinkElement) headLinkElement.href = link
}

export const themeCustomizer = (item: Record<Partial<keyof CustomizerType>, string | number>) => {
  ...
  if (colorKeys.includes(key as ColorType['variant'])) {
    ...
    const themeColorAndVariants: Record<string, string> = { [keyPrefix + key]: colorValue }
    themeColorAndVariants[keyPrefix + key + '-rgb'] = getRGBHex(colorValue)
    themeColorAndVariants[keyPrefix + key + '-btn-hover-active-border-bg-color'] = getHoverActiveOrBorderColor(colorValue, -10)
    if (key === 'primary') {
      themeColorAndVariants[keyPrefix + 'theme-link-hover-color'] = getHoverActiveOrBorderColor(colorValue, -10)
      themeColorAndVariants[keyPrefix + 'theme-accordion-btn-active-icon'] = `url("data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%2024%2024'%20fill='${colorValue.replace(/#/g, '%23')}'%3e%3cpath%20d='M.5%206.3c.6-.6%201.6-.6%202.3%200l9.3%209.3%209.3-9.3c.6-.6%201.6-.6%202.3%200%20.6.6.6%201.6%200%202.3L13.3%2018.8c-.6.6-1.6.6-2.3%200L.8%208.5c-.9-.8-.9-1.6-.3-2.2z'/%3e%3c/svg%3e")`
    }
  ...
  } else if (['rootFontSize', 'bodyFontSize'].includes(key)) {
    return { [keyPrefix + pascalCaseToKebabCase(key)]: value + 'rem' }
  ...
  } else if (['roundedSm', 'roundedLg', 'roundedXl', 'roundedXxl'].includes(key)) {
    ...
    return {
      [keyPrefix + 'border-radius-' + size]: `calc(var(${rootVariablePrefix}border-radius) * ${value})`,
