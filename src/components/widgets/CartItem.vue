<template>
  <div class="d-sm-flex align-items-center" :class="!idx ? 'pb-4' : idx === useShopping.state.cartItems.length - 1 ? 'border-top pt-4' : 'border-top py-4'">
    <router-link class="d-inline-block flex-shrink-0 bg-secondary rounded-1 p-sm-2 p-md-3 mb-2 mb-sm-0" :to="{ name: item.product?.link?.name, params: item.product?.link?.params }">
      <img :src="item.product?.images[0]" width="110" alt="Product" />
    </router-link>
